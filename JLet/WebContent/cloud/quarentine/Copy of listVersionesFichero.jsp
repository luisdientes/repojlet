			  	   		...
			  	   		<tr class="cab">
								...
								int indexfile = 0;
									...
									<td width="15%" align="center" class="tablaGrandeW">
										<table border="0">
											<tr>
												<td style="cursor:pointer" align="center" class="fonts6" width=25%><img id="<%=txnombre%>" ondblclick="abrirFicheroDirec('<%=tipofich %>','<%=txnombre %>','<%=tipoperm %>','<%=propieta %>')" onclick="cambiaImagen('<%=tipofich %>','<%=txnombre %>','<%=idinodox %>','<%=tipofile%>','<%=tipoperm %>','<%=propieta %>')" height=80px; width=80px; src="<%=imgfilex%>"></td>
											</tr>
											<tr>
