...
<%
	...
	if ((cdpantal != null) && (cdpantal.equals("altatien"))){
		txpantal = "Alta Tiendas";
 	}else if ((cdpantal != null) && (cdpantal.equals("listtien"))){
 		txpantal = "Listado Tiendas";
...
<script>
	...
	function enviaPantalla(idemisor,tpclient){
		...
		<% if ((cdpantal != null) && (cdpantal.equals("altatien"))){ %>
			altaTiendas(idemisor,tpclient);
		<% }else if ((cdpantal != null) && (cdpantal.equals("listtien"))){ %>
			ListadoTiendas(idemisor);
		<%}else if ((cdpantal != null) && (cdpantal.equals("mapatien"))){ %>
			MapaTiendas(idemisor);
		<% } %>
	
	}
	
	function altaTiendas(idemisor,tpclient){
		document.formMenu.services.value = "ListTiendasSrv";
		document.formMenu.view.value	 = "tiendas/altaTiendas.jsp";		
		document.formMenu.idemisor.value = idemisor;
		document.formMenu.tpclient.value = tpclient;
		document.formMenu.tpaccion.value = "AL";
		40.1558982
		document.formMenu.submit();
		
	}
	
	function ListadoTiendas(idemisor,tpclient){
		document.formMenu.services.value = "ListTiendasSrv";
		document.formMenu.view.value	 = "tiendas/listTiendas.jsp";		
		document.formMenu.idemisor.value = idemisor;
		document.formMenu.tpclient.value = tpclient;
		document.formMenu.submit();
	}
	
	function MapaTiendas(idemisor,tpclient){
		document.formMenu.services.value = "ListTiendasSrv";
		document.formMenu.view.value	 = "tiendas/mapasTiendas.jsp";		
		document.formMenu.idemisor.value = idemisor;
		document.formMenu.tpclient.value = tpclient;
		document.formMenu.submit();
	}
...
<body>	
	...
	<form method="POST" name="formMenu" action="/JLet/App">
		...
		<input type="hidden" name="tpaccion" 	value=""/>
	</form>
