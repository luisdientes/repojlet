...
<script>
	...
	<form method="POST" name="formMenu" action="/JLet/App">
		...
		<input type="hidden" name="services" value="ListAlbaranesWebEmisorSrv"/>
		<input type="hidden" name="view" value="factura/listadoAlbaranesWebEmisor.jsp"/>
		<input type="hidden" name="idemisor" value="<%=idemisor%>"/>
	...
	<form method="post" name="seleAlba" action="/JLet/App">
		<input type="hidden" name="controller" value="FacturaHttpHandler"/>
		<input type="hidden" name="services" value="VistaPreviaAlbaWebSrv"/>
		<input type="hidden" name="view" value="factura/vistaPreviaAlbaWeb.jsp"/>
		...
		<input type="hidden" name ="informad" value="">
	...
	<!-- INICIO FORMULARIO FILTRO -->
						...
						<% 
							...
							for (int i = 0; i < gridClie.rowCount(); i++){
								...
								if(idclient.equals(idclierc)) {%>
									<option value="<%=idclierc  %>" selected="true"  > <%= gridClie.getStringCell(i,"txnombre") %> <%= gridClie.getStringCell(i,"txapelli") %></option>
							<% }else{%>
		  							<option value="<%=idclierc  %>"  ><%= gridClie.getStringCell(i,"txnombre") %> <%= gridClie.getStringCell(i,"txapelli") %></option>
							 <%}
				    		 }
					 		%>  
					</select>	
		...
		<% 
		  ...
		  for (int i = 0; i < grfactur.rowCount(); i++){ 
			  	...
			  	informad = grfactur.getStringCell(i,"informad");
		  %>
				<tr class="usuario">
				<% String cddivisa = grfactur.getStringCell(i,"cddivisa"); %>
			
					<td class="fonts6" align="center" onclick="abrirFactura('<%=grfactur.getStringCell(i,"filecrea") %>');" style="cursor:pointer">
						<img src="/JLet/common/img/varios/factura.png" width="20px" height="25px" title="Albarán"/></td>
					<td class="fonts6"><%= grfactur.getStringCell(i,"preffact") %> <%= grfactur.getStringCell(i,"cdfactur") %></td>
					<td class="fonts6" align="center"><%= grfactur.getStringCell(i,"fhfactur") %></td>
					<td class="fonts6"><script>document.write(pintanombre(<%= grfactur.getStringCell(i,"idclient") %>));</script></td>
					<td class="fonts6" align="right"><%= grfactur.getStringCell(i,"baseimpo") %> <%= cddivisa %> &nbsp;&nbsp;&nbsp; </td>
					<%if(pendient > 0) {%>
					
					<td class="fonts6" align="center"><a class="boton" onclick="verAlbaranWeb(<%= idalbara %>,<%= grfactur.getStringCell(i,"idclient") %>,<%= grfactur.getStringCell(i,"aniofact") %>,'<%=cdfactur%>','<%=informad%>')">Generar Factura</a></td>
					
					 <% }else{ %>
					<td class="fonts6" align="center">FACTURADO</td>
					<% } %>
