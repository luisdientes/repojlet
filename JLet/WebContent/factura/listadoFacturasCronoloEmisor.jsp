...
function filtrafactura(){
	...
	function devuelveMes(mesfac){
		
		return nommes[mesfac];
	}
	...
	<form method="POST" name="formMenu" action="/JLet/App">
		...
		<input type="hidden" name="view" value="factura/listadoFacturasItbisEmisor.jsp"/>
		<input type="hidden" name="idemisor" value="<%=idemisor%>"/>
		...
		<input type="hidden" name="conitbis" value="S"/>
	</form>
	...
	<br>
		...
		<table align="center" class="tab-listado" width="100%" >
			<tr>
							...
							<td><div class="input-b1">&nbsp;</div></td>
							<td><div class="input-b1">Numero</div></td>
							<td><div class="input-b1">Tipo factura</div></td>
							<td><div class="input-b1"> NFC </div></td>
			...
			<% 
			  ...
			  for (int i = 0; i < grfactur.rowCount(); i++){ 
				  	...
				  	cdcronol = grfactur.getStringCell(i,"cdcronol"); 
					...
					//cddivisa = "&euro;";
			  		...
			  		mesfact = grfactur.getStringCell(i,"fhfactur");
			  		String aniofa = mesfact.substring(6);
		  			mesfact = mesfact.substring(3, 5);
				...
				<% }%>
					<% if(!logotpcl.equals("")){ %>
								<td class="fonts6jej"><div><img title="Logo" src="/JLet/common/img/icons/<%=logotpcl%>"></div></td>
							<%}else{ %>
							<td><div >&nbsp;</div></td>
							<%} %>
					...
					<td align="center" class="fonts6jej" style="font-size:12px" onclick="abrirFactura('<%=grfactur.getStringCell(i,"filecrea") %>');" style="cursor:pointer">
						<img src="/JLet/common/img/varios/factura.png" height="24px" title="Factura" style="cursor:pointer"/>
					</td>
					<td align="center" class="fonts6jej" style="font-size:12px"><%=cdcronol %></td>
				    <td align="center" class="fonts6jej" style="font-size:12px"><%=grfactur.getStringCell(i,"txtpfact") %></td>
				
				
				
					<td class="fonts6jej"><%= grfactur.getStringCell(i,"preffact") %> <%= grfactur.getStringCell(i,"cdfactur") %></td>
					<td align="center" class="fonts6jej" style="font-size:12px"><%= grfactur.getStringCell(i,"fhfactur") %></td>
					<td class="fonts6jej" style="font-size:12px"><%= grfactur.getStringCell(i,"rzsocial") %></td>
					<td align="right" class="fonts6jej" style="font-size:12px"><%= formateador.format(Double.parseDouble(grfactur.getStringCell(i,"baseimpo"))) %> <%= cddivisa %> &nbsp;&nbsp;&nbsp; </td>
					<td align="right" class="fonts6jej" style="font-size:12px"><%= formateador.format(Double.parseDouble(grfactur.getStringCell(i,"imptaxes"))) %> <%= cddivisa %> &nbsp;&nbsp;&nbsp; </td>
					<td align="right" class="highlightedColumn" style="font-size:12px"><%= formateador.format(Double.parseDouble(grfactur.getStringCell(i,"imptotal"))) %> <%= cddivisa %> &nbsp;&nbsp;&nbsp; </td>
					...
					<% if (permEmis.containsKey("REGEFACT")){ %>					
						<td style="cursor:pointer" onclick="regenera('<%= grfactur.getStringCell(i,"idfactur") %>')"></td>
					<% }  %>
					
					<% if (mcpagado.equals("S")){ %>					
						<td></td>
					<% } else{ %>
					<td style="cursor:pointer" onclick="marcapag('<%= grfactur.getStringCell(i,"preffact") %> <%= grfactur.getStringCell(i,"cdfactur") %>','<%= formateador.format(Double.parseDouble(grfactur.getStringCell(i,"imptotal"))) %>','<%= grfactur.getStringCell(i,"idfactur") %>','<%=cddivisa%>')"></td>
					<% }  %>
