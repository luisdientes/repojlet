...
<%
   ...
   if (request.getAttribute("io") != null ) {
		ObjectIO io = (ObjectIO)request.getAttribute("io");
			...
			try {
				numsegui	= io.getStringValue("numsegui");
			}catch(Exception ex){
				System.err.println("ERROR - Recibiendo numsegui");	
			}
			try {
				entregpo	= io.getStringValue("entregpo");
			}catch(Exception ex){
				System.err.println("ERROR - Recibiendo txcondpa");	
			}
			
			try {
				fhenviox	= io.getStringValue("fhenviox");
			}catch(Exception ex){
				System.err.println("ERROR - Recibiendo fhenviox");	
			}
			
			
			try {
				portetot	= io.getStringValue("portetot");
			}catch(Exception ex){
				System.err.println("ERROR - Recibiendo portetot");	
			}
	...
	<div class="centradoFac" style="width:80%">
		...
		<table cellspacing="10" border="0" align="center" class="tablaGrande">
				...
				<tr>
				  	...
				  	<td   style="width:200px;"><%=numsegui %></td>
			
				 <td class="input-b1" style="width:35%">Fecha Envio</td>
				  <td class="input-m" style="width:200px;" class="fonts6">
				  <%=fhenviox %>
				  </td>
				...
				<tr>
				  	...
				  	<td  width=20% class="fonts6" align="center"><input type="checkbox" value="S" id="mcpagado"></td>
				  	<td>&nbsp;</td>
					<td>&nbsp;</td>
				 </tr>
	  				...
	  				descuent	= Double.parseDouble(rsFact.getStringCell(j,"descuent"));
	  		 	 	...
	  		 	 	%>  	
				  		<tr>
					  	    ...
					  	    <td width="10%" class="fonts6"><%=formateador.format(precioun)%></td>
		...
		<table align="right" class="piefactu">
			...
			<tr id="muesrete" <%if(porcrete > 0){ %> style="table-row"<%}else{%>style="display:none" <%} %>>
				...
				<td class="fonts6"><input style="border:none" type="text" name="retencio" id="retencio" disabled class="fonts6" value="<%=formateador.format(impreten)%>"></td>
			</tr>
			<tr>
