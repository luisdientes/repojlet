...
public class FRAgenericESSrv extends Service {
    ...
    public void process(ObjectIO input, ObjectIO output){
    	...
    	try {
	        ...
	        namefile = cabecNFC + numerNFC + "_"+aniofact+".pdf";
...
// ------------------------------------------ START PINTAR DATOS -----------------------------
    ...
    public void pintaFecha(){
    	...
    	try {
    		...
    		PdfPTable table = new PdfPTable(new float[] { 0.5f, 1f, 0.5f});    		
            ...
            chunkNom = new Chunk(aniofact+ "/"+numerNFC+" "+cabecNFC, fontboldVCS);
            contcell = new Phrase(chunkNom);
    	...
    	try {
			...
			if(porReten != null && !porReten.equals("") && !porReten.equals("0.0")){
			    ...
			    if (imporret >0){
					...
					porcrete = Double.parseDouble(porReten) * 100;
					
					table.addCell(FRAparen.getCelda2(" ",fuente,new BaseColor(255,255,255),"center",0,2)).setBorderColor(new BaseColor(255,255,255));
					table.addCell(FRAparen.getCelda2("IRPF ("+ Math.round(porcrete) +"%)",fuenteTot,bkcolorCab,"center",1,2)).setBorderColor(new BaseColor(255,255,255));
					table.addCell(FRAparen.getCelda(" - "+formatDivi.format(imporret) + divisaSim,fuenteTot,bkcolor,"right")).setBorderColor(new BaseColor(255,255,255));
				}
