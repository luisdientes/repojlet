...
public class TestBigGrid {
    ...
    public static void main(String[] args) throws Exception {
        ...
        XSSFSheet sheet = wb.createSheet("BigGrid2");
        ...
        String sheetRef = sheet.getPackagePart().getPartName().getName(); //sheet.getPackagePart().getPartName().getName();
    ...
    private static Map<String, XSSFCellStyle> createStyles(XSSFWorkbook wb){
        ...
        style4.setDataFormat(fmt.getFormat("dd/mm/yyyy"));
    ...
    public static XSSFColor crearColor(int red, int green, int blue) {
    ...
    private static void generate(Writer out, Map<String, XSSFCellStyle> styles) throws Exception {
        ...
        sw.createCell(numColum++, "N. ACC.", styleIndex);
        sw.createCell(numColum++, "NOMBRE", styleIndex);
        sw.createCell(numColum++, "TIPO", styleIndex);
        sw.createCell(numColum++, "PAÍS", styleIndex);
        sw.createCell(numColum++, "N. ORDEN", styleIndex);
        sw.createCell(numColum++, "NOMBRE", styleIndex);
        sw.createCell(numColum++, "NIF", styleIndex);
        sw.createCell(numColum++, "MAIL", styleIndex);
        sw.createCell(numColum++, "POSICIÓN", styleIndex);
        sw.createCell(numColum++, "CD. POSTAL", styleIndex);
        sw.createCell(numColum++, "CALLE", styleIndex);
        sw.createCell(numColum++, "CIUDADesaksjfñasoidfjaosjifñasoidj", styleIndex);
        sw.createCell(numColum++, "% CAPITAL", styleIndex);

        sw.endRow();

        //write data rows
        for (int rownum = 15; rownum < 20; rownum++) {
            sw.insertRow(rownum);

            sw.createCell(0, "Hello, " + rownum + "!");
            sw.createCell(1, (double)rnd.nextInt(1000)/1000, styles.get("percent").getIndex());
            sw.createCell(2, (double)rnd.nextInt(100000), styles.get("entero").getIndex());
